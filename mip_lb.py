...
def get_lower_bound(relaxation_sol, depot, edges):
    ...
    for node in H.nodes:
        ...
        H.nodes[node]["demand"] = sum(
            H.edges[edge]["visited"] for edge in in_edges
        ) - sum(H.edges[edge]["visited"] for edge in out_edges)
    ...
    demand_nodes = {
        node: H.nodes[node]["demand"] for node in H if H.nodes[node]["demand"] != 0
    }

    if len(demand_nodes) == 0:
        # set x = ceil(x)
        new_solution["x"] = {
            edge: math.ceil(relaxation_sol["x"][edge]) for edge in relaxation_sol["x"]
        }
...
def callback(model, where):
    ...
    if (
        ...
        if (
            ...
            new_solution = get_lower_bound(relaxation_sol, model._depot, model._edges)
            ...
            for constr in model.getConstrs():
                ...
                if constr.Sense == GRB.GREATER_EQUAL and lhs < rhs:
                    return
                if constr.Sense == GRB.EQUAL and lhs != rhs:
                    return
