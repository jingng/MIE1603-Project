...
# Relaxation
for edge in edges:
   x[edge].setAttr('vtype', 'C')
   y[edge].setAttr('vtype', 'C')

for i,j in edges:
  if i<j:
    z[(i,j)].setAttr('vtype', 'C')
...
for i in nodes:
  for j in nodes:
      if (i,j) in edges:
        if x[(i,j)].x >= 0.05:
            G.add_edge(i,j, visited=float(x[(i,j)].x))
...
# If we didn't add any constraints
