...
class Map(folium.Map):
    ...
    **kwargs : keyword arguments, optional, default: no attributes
    ...
    def add_graph(self, G, plot_nodes=False, edge_color="#3388ff", edge_width=3,
                  ...
                  fill_opacity=1):
        """ Add the road network graph created with ``runtrack.graph.graph.graph_from_bbox`` method
    ...
    def draw_candidates(self, candidates, radius, point_radius=1, point_color="black", point_fill=True,
        ...
        ----------
        candidates: dict
            Candidates' dictionary computed via ``runtrack.matching.candidate.get_candidates`` method
        radius: float
    ...
    def draw_path(self, G, trellis, predecessor, path_name="Matched path", path_color="green", path_weight=4,
        ...
        G: networkx.MultiDiGraph
            Road network graph.
        trellis: nx.DiGraph
            Trellis DAG graph created with ``runtrack.matching.mpmatching_utils.create_trellis`` method
        predecessor: dict
            Predecessors' dictionary computed with ``runtrack.matching.mpmatching.viterbi_search`` method
        ...
        path_elabs = mpmatching_utils.create_path(G, trellis, predecessor)
        colors = deque(['red', 'blue', 'green', 'purple', 'orange', 'darkred','lightred', 
                  'beige', 'darkblue', 'darkgreen', 'cadetblue', 'darkpurple', 
                  'pink', 'lightblue', 'lightgreen'])

        for path_elab in path_elabs:
